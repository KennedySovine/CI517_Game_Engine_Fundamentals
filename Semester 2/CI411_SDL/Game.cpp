...
void Game::handleEvents()
{
	...
	case SDL_KEYDOWN:
		...
		switch (Game::playerInputEvent.key.keysym.sym)
		{ 
			// Pause Menu
			case SDLK_ESCAPE:
...
{
	easterEgg = false;
	// Create the pause screen background only once
	static GameObject* background = new GameObject("assets/images/Square_grey.png", 0, 0);
	background->setSize(1920, 1080);
		...
		std::string screenText = "GAME PAUSED\n\nPress 'r' to restart";
		screenText += "\n \n \n Press ESCAPE to resume the game";
		if (isEgg()) {
			screenText += "\n\nKENNEDY-SOVINE-CI517-2023-24";
		}
		textColour = { 0, 0, 0 };
		SDL_Rect textRect = { 250, 250,0,0 }; // start position of the text
		textSurface = TTF_RenderText_Blended_Wrapped(font2, screenText.c_str(), textColour, 0);
		...
		// Pause game loop
			...
			{
				// User requests quit
				if (playerInputEvent.type == SDL_QUIT)
				{
					replay = false;
					gameRunning = false;
					gamePaused = false;
				...
				// User presses a key
				else if (playerInputEvent.type == SDL_KEYDOWN)
				{
					// User presses escape
					if (playerInputEvent.key.keysym.sym == SDLK_ESCAPE)
					{
						gamePaused = false;
					}
					// User presses 'r' for restart
					else if (playerInputEvent.key.keysym.sym == SDLK_r)
					{
						resetAllObjects();
						loadMap(currentLevel);
						gamePaused = false;
					}
					else if (playerInputEvent.key.keysym.sym == SDLK_e)
					{
						toggleEgg();
					}
				}
